...
function floatALL($el) {
	if(!is_array($el)){ //определяет является ли переменная массивом
		$el=(float)$el;
	}
	else {
		$el=array_map('floatALL',$el); //Применяет callback-функцию ко всем элементам указанных массивов
	}
...
function hc($el) {
	if(!is_array($el)){ //определяет является ли переменная массивом
		...
		$el=array_map('hc',$el); //Применяет callback-функцию ко всем элементам указанных массивов
	}
	return $el;
}
